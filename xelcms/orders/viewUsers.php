...
if (confirm("Are you sure, you want to delete this record? It will delete all order(s), record(s) related with this user.")) {
return true;
}else{
return false;
}
...
echo "<span class=td>&nbsp;<img src=\"../dzimages/arrowpath.gif\" />&nbsp;Registered Users <img src=\"../dzimages/arrowpath2.gif\" />&nbsp;View</h2></div><br /><br></span>";
...
<?php
echo "<table width=\"100%\" align=\"right\"><tr bgcolor=\"#425d7b\" style='color:#fff' class=td><td align=left width=10%><font color=\"#ffffff\"><strong>Name</strong></font></td><td align=\"left\" halign=\"middle\" width=\"15%\"><font color=\"#ffffff\"><strong>Email</strong></font></td><td align=\"center\" halign=\"middle\" ><font color=\"#ffffff\" width=10%><strong>Address</strong></font></td><td align=left width=10%><font color=\"#ffffff\"><strong>Gender</strong></font></td><td align=left width=10%><font color=\"#ffffff\"><strong>Phone</strong></font></td><td align=\"center\" halign=\"middle\" width=10% ><font color=\"#ffffff\"><strong>Delete</strong></font></td></tr>
...
//print_r($row);
	...
	$edate=$row['name'];
	$email = $row["email"];
	$country = mysql_result(mysql_query("select country_name from shipping_master where id = ". $row["country"]),0);
	
	$status = $row["address"].", ". $row["city"].", ". $row["pin_code"]. ", ". $row["state"].", ". $country;
	
	$bg = ($bg=='#eeeeee' ? '#cccccc' : '#eeeeee');
	echo "<tr bgcolor=\"$bg\" class=td><td>$edate</td>
			<td halign=\"middle\">$email</td><td align=\"center\" halign=\"middle\">". stripslashes($status);
	echo "<td>".stripslashes($row["gender"])."</td><td>".stripslashes($row["phone"])."</td>";			
