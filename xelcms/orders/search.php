...
echo "<table width=\"100%\" align=\"right\"><tr bgcolor=\"#425d7b\" style='color:#fff' class=td><td align=left width=10%><font color=\"#ffffff\"><strong>Order Date</strong></font></td><td align=\"left\" halign=\"middle\" width=\"15%\"><font color=\"#ffffff\"><strong>Order Number</strong></font></td>
	...
	/*if($_GET["type"] == "sub-category")
		 ...
		 category  = ". $_GET["ordNum"];
	else*/
		
		$query="select * from orders, orders_master where orders.order_id = concat('SG-Ship-Order-',orders_master.order_id) and orders.order_id like '%". $_GET["ordNum"]."%' group by orders.order_id";
...
if ($num > 0) {
				if ($num_pages > 1) {
					...
					$current_page = ($start/$display) + 1;
					if ($current_page != 1) {
							echo '<a href="' . $_SESSION['PHP_SELF'] .'?o=' . $o . '&sts=' . ($start - $display) . '&np=' . $num_pages . '&ordNum='.$_GET["ordNum"].'&type='.$_GET["type"].'"  class=page>Previous</a>';
						}
						else {
						echo 'Previous';
						}
 	...
 	// Make all the numbered pages.
						for ($i = 1; $i <= $num_pages; $i++) {
							if ($i != $current_page) {
								echo '&nbsp;<a href="' . $_SESSION['PHP_SELF'] .'?o=' . $o . '&sts=' . (($display * ($i - 1))) . '&np=' . $num_pages . '&parentid='.$parentid.'&pid='.$pid.'&special='.$special.'&ordNum='.$_GET["ordNum"].'&type='.$_GET["type"].'" class="page" >' . $i . '&nbsp;</a>';
							} else {
...
for ($i = 1; $i <= $num_pages; $i++){
						...
						if ($current_page != $num_pages) {
							echo '<a href="' . $_SESSION['PHP_SELF'] .'?o=' . $o . '&sts=' . ($start + $display) . '&np=' . $num_pages . '&ordNum='.$_GET["ordNum"].'&type='.$_GET["type"].'"  class=page>Next</a>';
						}
						else {
				echo 'Next';
				}
