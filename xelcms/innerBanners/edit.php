...
echo "<span class=td><img src=\"../dzimages/arrowpath.gif\" />&nbsp;Inner Page(s) Banners</a> <img src=\"../dzimages/arrowpath2.gif\" />&nbsp;Edit</span><br><br>";
...
//hidCombination for update query above, don't know, yet...

$sql=mysql_query("update banners set  image_name ='".$i."', url = '".addslashes($_POST["url"])."',  banner_for = '".$newsFor."', banner_loc = '". $_POST["displayLoc"]."',  banner_active = '".$_POST["bannerActive"]."' where id = ".$_POST["hID"]) or die (mysql_error());
...
$arrNewsFor = explode("#",$rowDNews["banner_for"]);
...
<input type="hidden" name="hidCombination" value="<?php echo $rowDNews["banner_for"];?>" />
...
<?php } ?>			
	...
	<tr class="td">
		...
		<!--	<input type="checkbox" name="newsFor[]" value="ev" />Eastern Voice
			...
			<input type="radio" name="displayLoc" id="displayLoc" value="left" <?php if($rowDNews["banner_loc"] == "left") echo 'checked="checked"'; ?> />Left
		</td>
  	</tr>
	<tr class="td">
    	<td width="146">Banner For:</td>
	    <td width="670" style="line-height:40px;">
			<input type="checkbox" name="newsFor[]" value="ev" <?php if(in_array("ev",$arrNewsFor)) echo 'checked="checked"'; ?> />Eastern Voice
			<input type="checkbox" name="newsFor[]" value="sne" <?php if(in_array("sne",$arrNewsFor)) echo 'checked="checked"'; ?> />SikhTimes (English)
			<input type="checkbox" name="newsFor[]" value="snp" <?php if(in_array("snp",$arrNewsFor)) echo 'checked="checked"'; ?> />SikhTimes (Punjabi)
			<br>
			<input type="checkbox" name="newsFor[]" value="about" <?php if(in_array("about",$arrNewsFor)) echo 'checked="checked"'; ?> />About Us (Main Site)
			<input type="checkbox" name="newsFor[]" value="contact" <?php if(in_array("contact",$arrNewsFor)) echo 'checked="checked"'; ?> />Contact Us (Main Site)
			<input type="checkbox" name="newsFor[]" value="advertise" <?php if(in_array("advertise",$arrNewsFor)) echo 'checked="checked"'; ?> />Advertise (Main Site)
			<br>
			<input type="checkbox" name="newsFor[]" value="terms" <?php if(in_array("terms",$arrNewsFor)) echo 'checked="checked"'; ?> />Terms & Conditions (Main Site)
			<input type="checkbox" name="newsFor[]" value="privacy" <?php if(in_array("privacy",$arrNewsFor)) echo 'checked="checked"'; ?> />Privacy Policy (Main Site)			
		</td>
  	</tr>	
	<tr class="td">
